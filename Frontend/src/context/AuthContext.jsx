...
export const AuthProvider = ({ children }) => {
    ...
    const global_socket = () => {
        const ws = new WebSocket(`ws://localhost/ws/notifications/?token=${tokens.access}`)
        
        ws.onopen = () => {
            setSocket(ws);
            console.log('WebSocket connected');
            axios.get('http://localhost/api/auth/users/me/',
                {
                    headers: {
    ...
    const createSocket = ()=> {
        let ws = new WebSocket(`ws://localhost/ws/chat/?token=${tokens.access}`)
         ws.onopen = (e)=> {
             setChatSocket(()=>ws)
