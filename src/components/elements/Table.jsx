...
function PadelsOther({ position }) {
  const [key, getKeysClicked] = useKeyboardControls();
  const factore = useRef(0.5)
  const padels = useLoader(OBJLoader, 'pad2.obj')
  useFrame((state) => {
    // console.log(state)
    const move = ref.current.translation()
    const rotation = ref.current.rotation()
    ...
    move.x = Math.max(-2, Math.min(2, move.x))
    move.z = Math.max(-2.5+5, Math.min(-2.7 +4, move.z))
    ref.current.setNextKinematicRotation(rotation)
    ref.current.setNextKinematicTranslation(move)
  });
  const ref = useRef();
...
export default function Table() {
  return (
    <>
      ...
      <Padels position={[0, 0.3, 1.9-8]} />
      <PadelsOther position={[0, 0.3, -1.95 - 4]} />
      <Net />
    </>
  );
}
